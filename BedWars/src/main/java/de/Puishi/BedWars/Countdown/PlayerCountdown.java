...
public class PlayerCountdown {
    ...
    public void start(Player player) {
        start = Bukkit.getScheduler().scheduleSyncRepeatingTask(BedWars.getInstance(), new Runnable() {
            @Override
            public void run() {
                switch (countdown) {
                    case 30:
                    ...
                    case 0:
                        ...
                        for(Player all : Bukkit.getOnlinePlayers()){
                            spawnID++;
                            all.teleport(BedWars.getInstance().getLocationAPI().loadLocation(BedWars.getInstance().getMap() + "." + spawnID));
                        }
