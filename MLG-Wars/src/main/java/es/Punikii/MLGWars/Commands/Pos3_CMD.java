...
public class Pos3_CMD implements CommandExecutor {

    private MLGWars plugin;

    public Pos3_CMD(MLGWars MLGWars) {
        ...
        if(sender instanceof Player) {
            ...
            if(p.hasPermission("pixel.hadmin")) {
                if(args.length == 0) {
                    ...
                    LocationManager.setLocation("Pos3", p.getLocation());

                    LocationManager.saveLocations();

                    p.sendMessage(MLGWars.prefix + "§7Du hast den §eSpawn §7gesetzt!");
                } else {
                    p.sendMessage(MLGWars.prefix + "§7Nutze: §c/setpos3");
                }
            } else {
