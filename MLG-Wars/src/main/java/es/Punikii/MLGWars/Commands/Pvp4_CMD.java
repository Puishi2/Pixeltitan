...
public class Pvp4_CMD implements CommandExecutor {

    private MLGWars plugin;

    public Pvp4_CMD(MLGWars main) {
        ...
        if(sender instanceof Player) {
            ...
            if(p.hasPermission("pixel.hadmin")) {
                if(args.length == 0) {
                    ...
                    LocationManager.setLocation("Pvp4", p.getLocation());

                    LocationManager.saveLocations();

                    p.sendMessage(MLGWars.prefix + "§7Du hast den §cvierten pvp Spawn §7gesetzt!");
                } else {
                    p.sendMessage(MLGWars.prefix + "§7Nutze: §c/pvp4");
                }
            } else {
