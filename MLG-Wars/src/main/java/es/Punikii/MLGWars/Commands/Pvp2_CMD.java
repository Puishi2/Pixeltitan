...
public class Pvp2_CMD implements CommandExecutor {

    private MLGWars plugin;

    public Pvp2_CMD(MLGWars main) {
        ...
        if(sender instanceof Player) {
            ...
            if(p.hasPermission("pixel.hadmin")) {
                if(args.length == 0) {
                    ...
                    LocationManager.setLocation("Pvp2", p.getLocation());

                    LocationManager.saveLocations();

                    p.sendMessage(MLGWars.prefix + "§7Du hast den §czweiten pvp Spawn §7gesetzt!");
                } else {
                    p.sendMessage(MLGWars.prefix + "§7Nutze: §c/pvp2");
                }
            } else {
